 ...
 * @LastEditTime: 2022-09-08 17:52:01
 * @LastEditors: NMTuan
 * @Description: 
...
const mode = ['shuffle', 'loop', 'one'] // 播放模式
const activeMode = 'loop'    //当前播放模式
...
const fetchNextSong = () => {
    ...
    if (activeSong.value.id !== undefined && activeMode === 'one') {
        audio.value.play()
        return
    }
    ...
    if (activeSong.value.id !== undefined && activeMode === 'loop') {
    ...
    watch(data, (val) => {
        ...
        audio.value.src =
            storageUrl + activeSong.value.file.filename_disk
        audio.value.play()
    })
}
